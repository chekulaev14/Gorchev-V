...
Веб-сервис для металлообработки. Мультимодульная система — общее ядро, модули подключаются под заказчика. Масштабирование — отдельный инстанс и БД на каждого клиента (не multi-tenant). Код общий, данные изолированы.
...
## Связка: Beads + Superpowers + Template Bridge

Для крупных задач используем тройку плагинов:
1. Beads — трекинг задач, зависимости, параллельные сессии
2. Superpowers — методология (brainstorming → plan → TDD → review → verify)
3. Template Bridge — unified-workflow (склеивает 1+2 в конвейер) + каталог 600+ агентов-специалистов

Конвейер: задача в Beads → brainstorm → план с подзадачами → worktree → TDD → review → verify → close.
Агенты из каталога ставятся вручную: `npx claude-code-templates@latest --agent категория/имя --yes` → файл в .claude/agents/.

НЕ ЧИТАТЬ статью при старте сессии. Читать ТОЛЬКО если: пользователь попросил, или непонятно как работает связка трёх плагинов. Статья: https://habr.com/ru/articles/1017110/

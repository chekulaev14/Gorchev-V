...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (tab) {
      case "nomenclature":
        result = await setupService.importNomenclature(rows as Record<string, unknown>[]);
        break;
      case "stock":
        result = await setupService.importStock(rows as Record<string, unknown>[]);
        break;
      case "bom":
        result = await setupService.importBom(rows as Record<string, unknown>[]);
        break;
      case "routing":
        result = await setupService.importRouting(rows as Record<string, unknown>[]);
        break;
    }

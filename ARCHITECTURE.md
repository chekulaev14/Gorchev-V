...
Next.js 16 (App Router) + React 19 + TypeScript 5. Стилизация: Tailwind CSS 4 (oklch цвета, CSS variables, dark mode через `.dark` класс). UI-компоненты: shadcn/ui + Radix UI. Иконки: Lucide React. Тосты: Sonner. Валидация: Zod 4. Утилиты CSS: class-variance-authority + clsx + tailwind-merge.

### Роутинг

```
/                        → redirect на /warehouse
/terminal                → терминал (PIN, каталог, детали)
/warehouse               → склад (главный интерфейс)
  /nomenclature          → номенклатура
  /nomenclature/[id]     → карточка позиции
  /stock                 → остатки
  /assembly              → сборка
  /orders                → производственные заказы
  /operations            → складские операции
  /processes             → процессы
  /production            → мониторинг производства
  /builder               → конструктор изделий (edit mode)
  /deleted               → удалённые позиции (edit mode)
```

API routes: /api/auth/, /api/terminal/, /api/nomenclature/, /api/stock, /api/bom, /api/workers, /api/processes, /api/production-orders, /api/config, /api/health/.

### Лейауты и рендеринг

Root layout (layout.tsx): `<html lang="ru">`, ThemeProvider (dark/light, localStorage), Toaster (sonner, top-right, 4s). Все страницы — `"use client"`. Модульные лейауты: terminal/layout.tsx, warehouse/layout.tsx — оборачивают children в ErrorBoundary.

### Middleware

src/middleware.ts — перехватывает /api/*. Public routes без проверки: /api/auth/login, /api/terminal/auth, /api/health/*. Извлекает JWT из cookie `gorchev_session`, проверяет expiration (WORKER 15 мин, остальные 10 ч), проверяет RBAC по ROUTE_RULES. Передаёт контекст в headers: x-actor-id, x-actor-role, x-worker-id.

### Стейт-менеджмент

React Context, без внешних библиотек.

WarehouseContext — данные (items, balances, bomChildren, workers), auth (session, login, logout), UI state (editMode). Два уровня refresh: refresh() — балансы + BOM (после операций), refreshAll() — все данные (после CRUD). Hook: useWarehouse().

ThemeContext — light/dark, localStorage, проверка mount для предотвращения hydration mismatch. Hook: useTheme().

Локальный стейт: useState в табах/формах. Derived state вычисляется в orchestrator, не в presentation компонентах.

### Стили

Tailwind CSS 4 через @tailwindcss/postcss. globals.css: импорты tailwind + tw-animate-css + shadcn/tailwind.css. Цвета через CSS custom properties (oklch). @layer base: border/outline defaults. Никакого глобального CSS компонентов — всё через Tailwind классы. Анимации: tw-animate-css.

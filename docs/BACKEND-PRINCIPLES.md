...
Структурированные логи через pino (lib/logger.ts). Не использовать console.log/error/warn — только `log.info()`, `log.warn()`, `log.error()` из `@/lib/logger`.

requestId — уникальный идентификатор запроса, автоматически добавляется в каждую строку лога через AsyncLocalStorage. Генерируется в middleware, прокидывается через request headers.

Новые API routes: оборачивать в `withRequestId()` из `@/lib/logger`.

Новые сервисы: добавлять log.info на старт/конец ключевых операций, log.error на ошибки с контекстом (входные данные, что делали).

Логи пишутся в stdout → pm2 → `/var/log/gorchev-v/app.log`. Ротация через pm2-logrotate (50M, retain 7).
